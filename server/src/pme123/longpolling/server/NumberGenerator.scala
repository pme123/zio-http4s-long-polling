...
object NumberGenerator {
  ...
  trait Service[R <: Clock] {
    def nextNumbers(maxDuration: Duration, rts: Runtime[Random with Clock]): RIO[R, List[Int]]
  }

  object > extends Service[GeneratorEnv] {
    final def nextNumbers(maxDuration: Duration, rts: Runtime[Random with Clock]): ZIO[GeneratorEnv, Throwable, List[Int]] =
      ZIO.accessM(_.generator.nextNumbers(maxDuration, rts))
  }
  ...
  trait Live extends NumberGenerator {
    ...
    val generator: Service[GeneratorEnv] = new Service[GeneratorEnv] {
      ...
      def nextNumbers(maxDuration: Duration, rts: Runtime[Random with Clock]): RIO[GeneratorEnv, List[Int]] =
        for {
